using System.Security.Claims;
using ClinicFlow.Api.Features.Patients;
using MediatR;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace ClinicFlow.Api.Controllers;

[ApiController]
[Route("api/v1/patients")]
public class PatientsController : ControllerBase
{
    private readonly IMediator _mediator;

    public PatientsController(IMediator mediator)
    {
        _mediator = mediator;
    }

    [HttpGet("{id}")]
    [Authorize]
    public async Task<ActionResult<PatientRecordResponse>> GetPatientRecord(Guid id)
    {
        var userId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        var role = User.FindFirstValue(ClaimTypes.Role)!;

        var result = await _mediator.Send(new GetPatientRecordQuery(id, userId, role));
        return Ok(result);
    }
    [HttpPost("{id}/records")]
    [Authorize(Roles = "Doctor")]
    public async Task<ActionResult<AddMedicalRecordEntryResponse>> AddMedicalRecordEntry(Guid id, [FromBody] AddMedicalRecordEntryBody body)
    {
        var doctorId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        var result = await _mediator.Send(new AddMedicalRecordEntryCommand(id, doctorId, body.Notes, body.AppointmentId));
        return Ok(result);
    }
    [HttpGet("{id}/records")]
    [Authorize]
    public async Task<ActionResult<List<MedicalRecordEntryResponse>>> GetPatientMedicalHistory(Guid id)
    {
        var userId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        var role = User.FindFirstValue(ClaimTypes.Role)!;

        var result = await _mediator.Send(new GetPatientMedicalHistoryQuery(id, userId, role));
        return Ok(result);
    }
    [HttpGet]
    [Authorize(Roles = "Admin,Doctor,Receptionist")]
    public async Task<ActionResult<List<PatientListItemResponse>>> GetPatientsList()
    {
        var userId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        var role = User.FindFirstValue(ClaimTypes.Role)!;

        var result = await _mediator.Send(new GetPatientsListQuery(userId, role));
        return Ok(result);
    }

    [HttpPost]
    [Authorize(Roles = "Receptionist")]
    public async Task<ActionResult<RegisterWalkInPatientResponse>> RegisterWalkInPatient(RegisterWalkInPatientCommand command)
    {
        var result = await _mediator.Send(command);
        return Ok(result);
    }

    [HttpGet("me")]
    [Authorize(Roles = "Patient")]
    public async Task<ActionResult<PatientRecordResponse>> GetMyPatientRecord()
    {
        var userId = Guid.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
        var result = await _mediator.Send(new GetMyPatientRecordQuery(userId));
        return Ok(result);
    }
    public record AddMedicalRecordEntryBody(string Notes, Guid? AppointmentId);
}