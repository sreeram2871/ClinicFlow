...
- [ ] Implement Patients screen (list, profile, history, add note)
  - **[GAP FLAGGED]**: no GET /patients (list all) endpoint exists on the backend — only single-patient lookup was built. More involved than the earlier staff-list gap: Admin/Receptionist should see every tenant patient, but a Doctor should only see patients they've actually treated (per PatientAccessGuard's existing rule) — a flat unfiltered list would leak patient data a Doctor shouldn't see. Needs role-aware filtering, not a straight list.
